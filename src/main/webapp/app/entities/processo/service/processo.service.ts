...
type RestOf<T extends IProcesso | NewProcesso> = Omit<T, 'data'> & {
  data?: string | null;
};
...
export class ProcessoService {
  ...
  protected convertDateFromClient<T extends IProcesso | NewProcesso | PartialUpdateProcesso>(processo: T): RestOf<T> {
    return {
      ...processo,
      data: processo.data?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProcesso: RestProcesso): IProcesso {
    return {
      ...restProcesso,
      data: restProcesso.data ? dayjs(restProcesso.data) : undefined,
    };
  }
