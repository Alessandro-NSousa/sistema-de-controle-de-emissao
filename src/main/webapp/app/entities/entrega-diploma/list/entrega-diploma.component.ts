...
import { EntityArrayResponseType, EntregaDiplomaService } from '../service/entrega-diploma.service';
...
export class EntregaDiplomaComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEntregaDiploma): number => this.entregaDiplomaService.getEntregaDiplomaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.entregaDiplomas = this.refineData(dataFromBody);
  }

  protected refineData(data: IEntregaDiploma[]): IEntregaDiploma[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IEntregaDiploma[] | null): IEntregaDiploma[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.entregaDiplomaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
